...
# Per Property Lists with Individual Entries
flush = `redis-cli flushall`
start_time = Time.now
...
elapsed_time = Time.now - start_time
time_per_request = elapsed_time / REQUEST_COUNT
puts "Memory used by Per Property Lists (entries): #{memory_used} bytes #{memory_used / REQUEST_COUNT} bytes per request | #{elapsed_time} seconds | #{time_per_request} seconds per request"
